...
def transfer_to_cpp(instance: Instance, VC):
    ...
    lib.python_enter_point(instance.vertex_number(),
                           ...
                           [int(val) for val in VC])

def serialize_list_of_lists(lst):
    res = [len(lst), -2]
    for elem in lst:
        res += elem + [-1]
...
def transfer_to_cpp_new(instance: Instance, tw, decomp):
    buckets = [decomp.nodes().__iter__().__next__()]
    visited = set(buckets)
    tree = [[] for i in range(decomp.number_of_nodes())]
    
    def dfs(vertex, vertex_id):
        for u in decomp.adj[vertex]:
            if u not in visited:
                visited.add(u)
    ...
    import cffi
    ffi = cffi.FFI()
    ffi.cdef("void python_enter_point_ng(int n, int m, int* edges, int* buckets, int* tree);")
    
    lib = ffi.dlopen("./cppsolve.so")
    lib.python_enter_point_ng(instance.vertex_number(),
                              instance.edge_number(),
                              [instance.vertex_number() * a + b for (a, b) in instance.edges()],
                              serialize_list_of_lists(buckets),
                              serialize_list_of_lists(tree))

    
def solve_old(instance: Instance) -> Result:
    VC = [] #get_cover(instance)
...
def solve(instance: Instance) -> Result:
    ...
    nx_gr = networkx.Graph(instance.edges())

    (tw, decomp) = approximation.treewidth_min_degree(nx_gr)
    (tw_temp, decomp_temp) = approximation.treewidth_min_fill_in(nx_gr)

    if tw_temp < tw:
        (tw, decomp) = (tw_temp, decomp_temp)

    transfer_to_cpp_new(instance, tw, decomp)
